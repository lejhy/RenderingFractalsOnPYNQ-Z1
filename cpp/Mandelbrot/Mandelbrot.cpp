...
np::ndarray calculate(
    ...
    if (colours.shape(0) != number_of_colours) {
        throw std::invalid_argument(
            "Invalid number of colours, should be: " + number_of_colours
        );
    }
    ...
    boost::shared_ptr<cp::program_cache> global_cache;
    global_cache = cp::program_cache::get_global_cache(context);
    std::string options;
    std::string key = "__pynq_mandelbrot";
    cp::program program = global_cache->get_or_build(
        key,
        options,
        source,
        context
    ...
    cp::copy(device_vector.begin(), device_vector.end(), data, queue);
